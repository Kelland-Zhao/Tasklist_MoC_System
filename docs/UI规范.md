    ...
    <div class="container-fluid">
      <a class="navbar-brand" style="display:flex;align-items:center;gap:12px;text-decoration:none;">
        ...
        <span style="color:white;font-family:Arial,Helvetica,sans-serif;line-height:1.2;">
          <span style="font-size:xx-large;font-weight:bold;">页面中文名</span><br>
...
- body 背景 `#f5f6f8`
- navbar 必须有 Colgate logo（§3.1）+ 中上英下品牌名
...
| 场景 | 实现 |
|---|---|
| 操作成功 | `Swal.fire({icon:'success', title:swalTitle('成功','Success'), timer:1500, showConfirmButton:false})` |
| 操作失败 | `Swal.fire({icon:'error', title:swalTitle('失败','Fail'), html:swalHtml(详情cn, detailEn)})` |
| 密码错误 | `Swal.fire({icon:'error', title:swalTitle('密码错误','Password Error')})` |
| 危险操作（作废）前 | `Swal.fire({icon:'warning', showCancelButton:true, ...})` 二次确认 |
| 数据加载 | `Swal.fire({title:swalTitle('加载中...','Loading...'), allowOutsideClick:false, showConfirmButton:false, didOpen:()=>Swal.showLoading()})` |

**双语 toast helper**（每个 `_JS` 文件开头定义）：
```js
...
- 禁用 `alert()` / `confirm()`，统一用 SweetAlert2
...
[ ] body 背景 #f5f6f8
[ ] navbar 有 Colgate logo + 中上英下品牌名
[ ] 依赖走 CDN（不引入 Kez_*），SweetAlert2 用 include
[ ] 表头红底白字 sticky；双语用 <br>（中上英下）
